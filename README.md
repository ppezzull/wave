...
wave runs a vertical pipeline (read it as 7 OSI layers). A strategist types plain intent; the LLM only parses it into a bounded form; a **deterministic compiler** turns that into verified SwapVM bytecode that settles live on **Aqua**. The shipped strategy gets a public description and appears in an **X-style feed**. Takers discover it via **ENS** and verify the on-chain program matches its recorded hash before settling.
...
5. **Ship** — deploy the live Aqua order (liquidity stays in the maker wallet)
6. **Index** — The Graph subgraph emits deltas that drive the autonomous retune
7. **Settle** — taker resolves the ENS name, checks the program hash, then settles

```mermaid
block-beta
    columns 1

    L7["L7 · PRESENTATION\nNext.js SSR · X-style feed · safety card\nPrivy auth (id = ENS name or 0x) · Supabase social"]
    ...
    L3["L3 · SETTLEMENT — Aqua\nlive order · liquidity stays in-wallet · pull/push"]
    L2["L2 · DATA — The Graph\nfirst-party subgraph · indexes Swapped · retune signal"]
    L1["L1 · IDENTITY — ENS\nsubname + v0.programhash text record · discoverable · tamper-checked"]
...
|---|---|---|---|
| **L7** Presentation | social UI + safety card | Next.js (App Router, SSR), Privy, Supabase | discover, follow, like, trade |
...
| **L3** Settlement | live order | 1inch Aqua | custody never leaves the maker wallet |
| **L2** Data | retune signal | The Graph subgraph | on-chain truth, pure event-indexing |
| **L1** Identity | trust root | ENS subnames + `v0.programhash` | the on-chain program matches its recorded hash |

## Sponsor Chains

| Sponsor | Role |
|---------|------|
| **1inch** | SwapVM + Aqua — the core VM and settlement layer; extended with 2 custom opcodes |
| **The Graph** | First-party subgraph indexing `Swapped` — the agent's live retune signal + on-card stats |
| **ENS** | Subnames carrying a `v0.programhash` text record takers verify before settling (tamper-check, not cosmetic) |

> **Data split (intentional):** on-chain truth (swaps, hashes, volume) → **The Graph (L2)**; off-chain social (descriptions, follows, likes) → **Supabase (L7)**; joined at the API by strategy id.
...
- **X-style social feed** — public descriptions, follows, likes, trade-against
...
- **Solidity 0.8.30** / Foundry — SwapVM (forked 1inch, release/1.1) + 2 custom opcodes + `StrategyFactory`
- **1inch Aqua** — settlement; `ship()`/`dock()`/`pull()`/`push()` (custody stays in the maker wallet)
- **TypeScript** — deterministic compiler (Zod → AST → IR → bytecode) + agent (viem, `@1inch/aqua-sdk`)
- **Next.js** (App Router, SSR) + React — split-screen UI + safety card
- **The Graph** — first-party subgraph (AssemblyScript mapping, GraphQL)
- **ENS** — ENSIP-25/26 + custom `v0.programhash` text record
- **Foundry Agents SDK** + **z.ai** — intent parsing
- **Privy** — wallet auth (id = ENS name or `0x…`)
- **Supabase** — off-chain social data (descriptions, follows, likes)
- **anvil** — mainnet fork for all demo runs
