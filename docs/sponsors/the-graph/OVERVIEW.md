...
⚠️ **Qualification posture (post-PR-#4, updated):** the demo runs on **live Sepolia** (no anvil fork, no mock) — see [PROD-TESTNET.md](../../strategy/PROD-TESTNET.md). The "live data from a Graph provider" requirement is satisfied cleanly: we publish the subgraph to **Subgraph Studio → decentralized network** (the primary path) and the demo reads live `Swapped` entities from it. **Residual risk to confirm at G1 (h12):** whether the decentralized network reliably indexes Sepolia EVM (chain 11155111); the supported-networks page lists only `Sepolia Beacon` in its default view. Fallback if not: one self-hosted `graph-node` container pointed at our Sepolia RPC (Nuthatch-style self-hosting is explicitly acceptable per the track rules). Either way the submission carries a Studio link.
