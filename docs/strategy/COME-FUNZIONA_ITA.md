...
- **`_oracleGuard2D`** — circuit breaker **maker-protection**: revert/clamp quando il prezzo implied devia da Chainlink oltre `maxDeviationBps`; halt totale se oracle stale. **Parte da `OraclePriceAdjuster.sol`** (già in repo, **non cablato in nessuna tabella**) ma ribaltato: quello è *taker-favored* (muove il prezzo verso l'oracle a favore del taker), il nostro è *maker-protection* (rifiuta/clampa i fill maker-unfavorable).
