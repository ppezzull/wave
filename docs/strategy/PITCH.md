...
- **"Is this real or a demo trick?"** → real Sepolia state; on-chain numbers come from live chain calls; the oracle that *triggers* the circuit breaker in the demo is a `MockAggregatorV3` we deploy on Sepolia — disclosed on the slide — because you cannot move a real Chainlink feed on demand, and demonstrating the breaker requires moving the market. Every demo moment is a real tx on Etherscan; repo history shows the whole build.
