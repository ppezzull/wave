...
**Stage discipline:** **no canned twins, no `DEMO_LIVE=0`, no anvil fork** — every beat is live Sepolia. **The un-cuttable moments: the live `ship()`/`swap()` and the judge-triggered halt** — those are the rubric-killers. Backup: a second funded wallet + backup Sepolia RPC URL (no fork to swap to). LLM stall → 1500ms watchdog retries live (disclosed); there's no recording to fall back to. **Cut plan if timing slips:** narrate the on-screen state and move on; **never** cut the live on-chain action, the reject-and-rewrite WOW beat, or the ENS discovery panel (those clear 1inch, WOW, and ENS respectively).
