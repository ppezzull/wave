# Tech Stack — Strategy Compiler (SwapVM)

_One-page reference for the team and the submission README. Source of truth for "what we build with." Aligned with [10-10-PLAYBOOK.md](./10-10-PLAYBOOK.md)._

> **Single biggest constraint:** this is a multi-language, multi-tool surface for 3 people in 36h. Solidity + TypeScript + AssemblyScript (subgraph) + React (Next.js) + GraphQL. The non-core element with a pre-agreed escape hatch is the **subgraph** (fallback: `eth_getLogs`).
...
```
┌──────────────────────────────────────────────────────────┐
│  UI (P3)          Next.js (App Router) + React, SSR       │  split-screen + safety card
│                   server actions hold keys + LLM call      │
├──────────────────────────────────────────────────────────┤
│  Compiler (P2)    TypeScript — Zod → AST → IR → bytecode   │  deterministic, reject-and-rewrite
├──────────────────────────────────────────────────────────┤
│  Agent (P2)       viem + @1inch/aqua-sdk                  │  ship/dock/monitor, ENS resolve+verify
├──────────────────────────────────────────────────────────┤
│  Data (P2)        The Graph — first-party subgraph        │  indexes Swapped → delta → retune
│                   (graph-node on fork; eth_getLogs fb)     │  (Studio key fallback; x402 optional)
├──────────────────────────────────────────────────────────┤
│  Identity (P2)    ENS — ENSIP-25/26 text records          │  program-hash verify, subnames
├──────────────────────────────────────────────────────────┤
│  On-chain (P1)    Solidity 0.8.30 / Foundry               │  SwapVM + 2 custom opcodes + StrategyFactory
│                   Aqua (1inch) for settlement              │
└──────────────────────────────────────────────────────────┘
        ▲ all demo runs against a mainnet FORK (anvil)
```
...
### On-chain core (P1) — Solidity / Foundry
...
### UI (P3) — Next.js (App Router) + React, SSR
- **Next.js with Server-Side Rendering** (App Router). Three panes: intent (NL sentence) / bytecode (hex, tokenized into `[op][len][args]`) / safety card (green/red verdict from the `quote()` battery). Plus an ENS-discovery pane.
- **Why SSR / server components:** keeps the **LLM call, API keys (Studio/x402), and the compile invocation server-side** — no secrets or heavy logic shipped to the browser, and the first paint can render a cached/canned safety card even before the live compile returns (this is the latency-fallback mechanism).
- **Server actions / route handlers** bridge to P2's compiler (`/compile`) + simulator (`/simulate`). The 1500ms watchdog swaps the live SSE stream to a canned `replay.json` on timeout (disclosed cached-but-real).
- Runs against the **anvil fork RPC**; the fork does the real work, the UI is a view layer.

### Agent / off-chain (P2) — TypeScript
...
The **subgraph** (AssemblyScript + local `graph-node` syncing a fork). If it fails at 3am, the Move #5 fallback is `eth_getLogs` — same retune behavior, labeled "subgraph syncing." Everything else in the stack is either core (Solidity/TS compiler — defend at all costs) or has a trivial fallback (Studio key for x402, canned replay for the live compile).
