...
| `wave.following/<strategy>` | the *follower*, on the follower's own name | a follow = one such record; count = follow graph |
...
| **Sepolia congestion / tx latency** | anvil blocks were instant; Sepolia is a real network with real block times (~12s) and occasional congestion. | Budget ~12–60s per on-chain beat in the demo script; never demo a flow that needs sub-block confirmation. Pre-broadcast the retune tx slightly before narrating it. |
