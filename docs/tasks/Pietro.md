...
> | `followers` | count of ENS `wave.following/<thisStrategy>` records pointing here | **ENS resolve** (resolve every name, scan text records) |
>
> **Why returnPct not raw PnL:** "Up 312%" ranks over "made $40k" — raw PnL is a whale leaderboard; return-percentage is the strategy-quality signal. **Why log2(followers):** a follower is a nudge, not a multiplier — the 100th follower matters less than the 1st. **Every term is sourced from chain or ENS → still no database.**
>
...
**Dealbreaker:** the autonomous zero-click retune provably driven by a **live Sepolia subgraph entity delta** (log cites the entity ID) + a demo that never dead-airs. A time/button-triggered retune scores ~0 on the Graph judge's "effective use" weight; a stage-melt loses the finalist slot for everyone.
