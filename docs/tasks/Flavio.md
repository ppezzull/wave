...
Owns identity + the agentic brain: ENS agent side (resolveVerify, register, program-hash verify) + the agentic layer (**Mastra + z.ai + custom MCP**, in its own container — see [AGENT.md](../strategy/AGENT.md)) — parse intent → bounded spec; read subgraph → `policy.decide()` → call Flaviano's `dock()`/`ship()`. Card payloads owed to Pietro per [frontend.md](../strategy/frontend.md).

| ☐ | Task | Person | Collab |
|---|---|---|---|
| ☐ | Stand up the Mastra agent skeleton in its own container (Mastra registry + `MCPServer` + z.ai via the AI SDK OpenAI-compatible provider). Smoke: one NL prompt → Zod-validated bounded form (LLM writes no code). Confirm against Flaviano's frozen `specVersion: 1` | Flavio | ← Flaviano (spec) |
...
| ☐ | `agent/src/monitor/graphDelta.ts` — poll the live Sepolia subgraph endpoint; runs `policy.decide()` on each tick (no fixtures, no canned path) | Flavio | ← Pietro (endpoint) |
| ☐ | `recompileAndShip()` action arm — wrap `dock()`→recompile (Flaviano's compiler)→`ship()` into the autonomous `retune` MCP tool (only `retuneAgent` holds it; never HITL-gated) | Flavio | ← Flaviano (signature + router address) |
