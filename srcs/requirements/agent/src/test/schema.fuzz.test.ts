...
// unhandled exception. (Flavio.md task #17.) No LLM, no I/O.
...
describe("StrategySpec.parse is total (G3 fuzz)", () => {
  const shapes: unknown[] = [
    ...
    { blocks: [{ type: "maker-fee", bps: -1 }] },
    { blocks: [{ type: "maker-fee", bps: 99999 }] },
    {
      specVersion: 1,
  ...
  it("random shapes never throw", () => {
    ...
    const types = ["deadline", "oracle-guard", "maker-fee", "curve", "salt", "bogus", 3, null];
    for (let i = 0; i < 300; i++) {
